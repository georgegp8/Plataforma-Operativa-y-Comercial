...
```bash
# Limpieza de instalaciones previas fallidas
...
```
F) Configuración del Backend (Laravel y .env)
...
Este paso es obligatorio para que Nginx tenga archivos visuales que mostrar. El comando npm run build es el encargado de fabricar la carpeta dist.

```bash
cd /var/www/Nubofact-Web-y-Facturador/frontend

# 1. Configurar ruta relativa de la API (Evita errores CORS)
sudo bash -c 'echo "VITE_API_URL=/api" > .env'

# 2. Instalar librerías de Node
sudo npm install --legacy-peer-deps

# 3. ¡OBLIGATORIO! Esto crea la carpeta 'dist' con los archivos compilados
sudo npm run build

# 4. Otorgar permisos a la carpeta generada para que Nginx pueda leerla
...
Configurar Nginx para que sirva los archivos de la carpeta dist y procese la API en el mismo puerto.
...
Solución: Volver al paso G y asegurar que npm run build se ejecutó correctamente y la carpeta tiene permisos www-data.
